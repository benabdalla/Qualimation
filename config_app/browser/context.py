...
from playwright.async_api import (
    ...
    FrameLocator,
    Page,
    Dialog,  # Ajouté pour résoudre 'Unresolved reference Dialog'
)

from config_app.browser.views import (
    BrowserError,
    BrowserState,
    TabInfo,
    URLNotAllowedError,
)
from config_app.dom.service import DomService
from config_app.dom.views import DOMElementNode, SelectorMap
from config_app.utils import time_execution_async, time_execution_sync

if TYPE_CHECKING:
    from config_app.browser.browser import Browser
...
class BrowserContextConfig:
    ...
    wait_sleep_step: float = 0.5  # Ajouté pour résoudre 'Unresolved reference WAIT_SLEEP_STEP'
...
class BrowserContext:
    ...
    @time_execution_async('--accept_alert')
    async def accept_alert(self):
        """Accepte une alerte JavaScript (alert, confirm, prompt)."""
        page = await self.get_current_page()

        # Utilisation de self.config.wait_sleep_step au lieu de WAIT_SLEEP_STEP
        await asyncio.sleep(self.config.wait_sleep_step)

        # Attendre l'apparition d'une alerte et l'accepter
        async with page.expect_dialog() as dialog_info:
            # Typage correct avec Dialog
            dialog: Dialog = await dialog_info.value
            await dialog.accept()

        logger.info("Click Accept on Alert.")

    @time_execution_async('--dismiss_alert')
    async def dismiss_alert(self):
        """Rejette une alerte JavaScript (alert, confirm, prompt)."""
        page = await self.get_current_page()

        # Utilisation de self.config.wait_sleep_step au lieu de WAIT_SLEEP_STEP
        await asyncio.sleep(self.config.wait_sleep_step)

        # Attendre l'apparition d'une alerte et la rejeter
        async with page.expect_dialog() as dialog_info:
            # Typage correct avec Dialog
            dialog: Dialog = await dialog_info.value
            await dialog.dismiss()

        logger.info("Click Dismiss on Alert.")
