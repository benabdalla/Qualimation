...
def main():
    ...
    if request.method == 'POST':
        ...
        if 'start' in request.form or 'execute_module_task' in request.form:
            ...
            Thread(target=run_task, args=(task_id, task, url, add_infos, max_steps, headless, use_vision, test_key)).start()
...
@app.route('/module/twitter/<action>')
def twitter_module(action):
    if 'user' not in session:
        return redirect(url_for('login'))
    if action == 'create':
        session['prefilled_task'] = "create a twitter account"
        return redirect(url_for('main'))
    flash(f"Twitter {action} not implemented!", 'info')
...
async def execute_task(task, url, add_infos, max_steps, headless, use_vision, test_key, task_id):
    ...
    try:
        ...
        if running_tasks.get(task_id) == "cancelled":
            ...
            if history and history.history and history.history[-1].model_output and history.history[-1].model_output.action:
                last_action = history.history[-1].model_output.action[0]
                action_json = json.loads(last_action.model_dump_json(exclude_unset=True))
                if (action_json.get("done") and "success" in action_json["done"] and action_json["done"]["success"]):
